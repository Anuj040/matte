    ...
    def forward(
        ...
        if self.mode != "full":
            ...
            if idx[0].size(0) > 0:
                ...
                y = F.interpolate(
                    src_bgr, (H_half, W_half), mode="bilinear", align_corners=False
                )
                ...
                out = F.interpolate(
                    out, (H_full, W_full), mode="bilinear", align_corners=False
                )
                ...
                fgr = F.interpolate(
                    fgr, (H_full, W_full), mode="bilinear", align_corners=False
                )
        else:
            x = torch.cat([hid, pha, fgr], dim=1)
            ...
            fgr = x[:, 1:]
            ref = torch.ones(
                (src.size(0), 1, H_quat, W_quat), device=src.device, dtype=src.dtype
            )
    ...
    def crop_patch(
        ...
        if self.patch_crop_method == "unfold":
            ...
            return torchvision.ops.roi_align(
                x, boxes, size + 2 * padding, sampling_ratio=1
            )
        else:
